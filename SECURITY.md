...
Public binaries through v1.6.0 are not Authenticode-signed and the per-user installation directory is user-writable. The UAC prompt therefore provides consent but not an OS-verified publisher identity. A SignPath Foundation application has been submitted, and SpaceLens has prepared a verifiable two-stage workflow that signs the app before embedding it into Setup and signs Setup afterward; see the [code signing policy](CODE_SIGNING_POLICY.md). Until a release explicitly states that both files are signed, users should install only from this repository's Releases page and verify the published SHA-256 files when downloading manually.
